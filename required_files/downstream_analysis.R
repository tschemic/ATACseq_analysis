...
##### Differential ATACseq analysis with edgeR ###############################################################
...
number_replicates = 3 # change accordingly
cpm_cutoff = 1 # change to filter aout peaks with low read number
control = "YPD" # change accordingly
treatment = "H2O2" # change accordingly
...
keep <- rowSums(cpm(d)> cpm_cutoff) >= number_replicates
d <- d[keep,]  ### filters out peaks with low number of reads
...
et <- exactTest(d, pair=c(control, treatment)) # this performs the differential signal analysis
...
detags <- rownames(d)[as.logical(de)] # this saves the row names of differentially expressed genes as "detags"
...
res <- as.data.frame(topTags(et, n=Inf)) # this saves the fold change table in "res"

### Create Vulcano plot ########################
pdf(file = "Vulcano_plot.pdf")
...
cpmill = as.data.frame(d$pseudo.counts) # this saves the counts per million table in a new R object
colnames(cpmill) <- d$samples$group

### PCA analysis with CPM values ###########################
library(ggbiplot)
